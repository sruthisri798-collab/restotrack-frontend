...
export default function Transactions() {
  ...
  const orphanRows = insights.filter(i =>
    i.type === 'match' && isOrphan(i.status)
  )
  ...
  const txnRows = transactions.map(t => {
    ...
    return {
      txn_id:           t.id,
      match_id:         insight.match_id || null,
  ...
  const allResolved  = counts.missing === 0 && counts.review === 0 && counts.orphan === 0
  ...
  // Sort — urgent first, waived last
  const urgencyOrder = { critical: 0, high: 1, medium: 2, low: 3 }
  filtered = [...filtered].sort((a, b) => {
    if (isWaived(a.status) && !isWaived(b.status)) return 1
    if (!isWaived(a.status) && isWaived(b.status)) return -1
    const ua = urgencyOrder[getUrgency(a.days_pending || 0)] ?? 4
    const ub = urgencyOrder[getUrgency(b.days_pending || 0)] ?? 4
    return ua - ub
  })
    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden', fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      ...
      {showExport && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'white', borderRadius: 12, padding: 24, width: 340, boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }}>
            ...
            <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 16 }}>Download all transactions as CSV for audit compliance. Leave dates blank to export everything.</div>

            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 11, color: '#374151', marginBottom: 4 }}>From date (optional)</div>
              <input type="date" value={exportFrom} onChange={e => setExportFrom(e.target.value)} style={{ width: '100%', border: '0.5px solid #e8e6e1', borderRadius: 6, padding: '7px 9px', fontSize: 11, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
            </div>

            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: '#374151', marginBottom: 4 }}>To date (optional)</div>
              <input type="date" value={exportTo} onChange={e => setExportTo(e.target.value)} style={{ width: '100%', border: '0.5px solid #e8e6e1', borderRadius: 6, padding: '7px 9px', fontSize: 11, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
            </div>

            <div style={{ background: '#f8f8f7', borderRadius: 8, padding: '10px 12px', marginBottom: 16, fontSize: 11, color: '#6b7280' }}>
              📋 Report includes: date, merchant, amount, category, status, receipt uploaded, resolved by, resolution reason, notes
            </div>

            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setShowExport(false)} style={{ flex: 1, background: '#f3f4f6', color: '#374151', border: 'none', borderRadius: 7, padding: '9px', fontSize: 12, cursor: 'pointer' }}>
                Cancel
              </button>
              <button onClick={handleExport} disabled={exporting} style={{ flex: 1, background: '#111827', color: 'white', border: 'none', borderRadius: 7, padding: '9px', fontSize: 12, cursor: exporting ? 'default' : 'pointer', fontWeight: 500, opacity: exporting ? 0.7 : 1 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <div style={{ padding: '12px 18px', background: 'white', borderBottom: '0.5px solid #e8e6e1', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => setShowExport(true)} style={{ background: 'white', color: '#374151', border: '0.5px solid #e8e6e1', borderRadius: 6, padding: '6px 14px', fontSize: 11, cursor: 'pointer', fontWeight: 500 }}>
              ⬇ Export
            </button>
            <button onClick={loadData} style={{ background: '#111827', color: 'white', border: 'none', borderRadius: 6, padding: '6px 14px', fontSize: 11, cursor: 'pointer', fontWeight: 500 }}>
              ↻ Refresh
            </button>
          </div>
        </div>

        {allResolved && counts.waived === 0 ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            <div style={{ fontSize: 40 }}>🎉</div>
            <div style={{ fontSize: 15, fontWeight: 500, color: '#111827' }}>All caught up!</div>
            <div style={{ fontSize: 12, color: '#9ca3af' }}>{transactions.length} transactions fully reconciled</div>
          </div>
        ) : (
          <>
            {/* Attention strip */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 8, padding: '10px 18px', background: 'white', borderBottom: '0.5px solid #e8e6e1', flexShrink: 0 }}>
              {[
                ...
                <div key={a.key} onClick={() => setFilter(filter === a.key ? 'all' : a.key)} style={{
                  ...
                  background: filter === a.key ? '#f0f0f0' : '#f8f8f7',
                  border: `0.5px solid ${filter === a.key ? '#374151' : '#e8e6e1'}`,
                  borderRadius: 9, padding: '8px 10px', cursor: 'pointer',
                }}>
                  <div style={{ width: 28, height: 28, borderRadius: 7, background: a.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0 }}>
                    {a.icon}
                  </div>
                  <div>
            ...
            {counts.waived > 0 && filter !== 'waived' && (
              <div onClick={() => setFilter('waived')} style={{ margin: '8px 18px 0', background: '#fafafa', border: '0.5px solid #e8e6e1', borderRadius: 9, padding: '9px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', flexShrink: 0 }}>
                <span style={{ fontSize: 12, color: '#374151', fontWeight: 500 }}>
                  📋 {counts.waived} transaction{counts.waived > 1 ? 's' : ''} manually closed — review for audit compliance
                </span>
                <span style={{ fontSize: 11, color: '#6b7280' }}>Review →</span>
              </div>
            )}

            {/* Refunds green banner */}
            {counts.refunds > 0 && filter !== 'refunds' && filter !== 'waived' && (
              <div onClick={() => setFilter('refunds')} style={{ margin: '8px 18px 0', background: '#f0fdf4', border: '0.5px solid #bbf7d0', borderRadius: 9, padding: '9px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', flexShrink: 0 }}>
                <span style={{ fontSize: 12, color: '#065f46', fontWeight: 500 }}>
                  ↩️ {counts.refunds} credit{counts.refunds > 1 ? 's' : ''} totalling ${Math.abs(totalCredits).toFixed(2)} — verify these are accounted for
                </span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 18px', background: 'white', borderBottom: '0.5px solid #e8e6e1', flexShrink: 0, marginTop: 8, flexWrap: 'wrap' }}>
              {['all', 'missing', 'review', 'orphan', 'refunds', 'matched', 'waived'].map(f => (
                <button key={f} onClick={() => setFilter(f)} style={{
                  ...
                  border: `0.5px solid ${filter === f ? '#111827' : '#e8e6e1'}`,
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                  ...
                  <tr>
                    <th style={{ width: 40, padding: '9px 0 9px 18px' }}>
                      <input type="checkbox"
                        ...
                        onChange={() => setSelectedIds(selectedIds.size === filtered.length ? new Set() : new Set(filtered.map(i => i.txn_id)))}
                ...
                <tbody>
                  {filtered.map((item, i) => {
                    ...
                    return (
                      <tr key={`${item.txn_id}-${i}`}
                        onClick={() => setSelected(isSel ? null : item)}
                        style={{
                          background: isSel ? '#f0f4ff' : waived ? '#fafafa' : urgency === 'critical' && isMissing(item.status) && !refund ? '#fff8f8' : 'white',
                      ...
                      >
                        ...
                        <td style={{ padding: '10px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                            ...
                            <div style={{ width: 32, height: 32, borderRadius: 8, background: '#f8f8f7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>
                              {getCategoryIcon(item.category)}
                            </div>
                            <div style={{ minWidth: 0 }}>
                              <div style={{ fontSize: 12, fontWeight: 500, color: waived ? '#9ca3af' : '#111827', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 150 }}>
                                {item.vendor}
                              </div>
                              {waived && item.resolution_action && (
                                <div style={{ fontSize: 10, color: '#9ca3af' }}>{item.resolution_action.replace(/_/g, ' ')}</div>
                              )}
                              {urgency === 'critical' && isMissing(item.status) && !refund && (
                                <div style={{ fontSize: 10, color: '#dc2626', fontWeight: 500 }}>Overdue — {item.days_pending}d</div>
                              )}
                              {urgency === 'high' && isMissing(item.status) && !refund && (
                                <div style={{ fontSize: 10, color: '#ea580c' }}>Urgent — {item.days_pending}d</div>
                              )}
                              {isOrphan(item.status) && (
                                <div style={{ fontSize: 10, color: '#8b5cf6' }}>No transaction found</div>
                              )}
                            </div>
                          </div>
                        </td>
                        <td style={{ fontSize: 11, color: '#9ca3af', padding: '10px' }}>{item.category || '—'}</td>
                        <td style={{ fontSize: 12, fontWeight: 500, color: refund ? '#059669' : waived ? '#9ca3af' : '#111827', padding: '10px' }}>
                          {refund ? '-' : ''}${Math.abs(item.amount || 0).toFixed(2)}
                        </td>
                        <td style={{ padding: '10px' }}>
                          <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 99, fontWeight: 500, background: refund ? '#ccfbf1' : cfg.bg, color: refund ? '#065f46' : cfg.color, whiteSpace: 'nowrap' }}>
                            {refund ? 'Refund' : cfg.label}
                          </span>
                        </td>
                        <td style={{ fontSize: 11, color: '#9ca3af', padding: '10px', whiteSpace: 'nowrap' }}>
                          {(item.transaction_date || item.receipt?.date)?.slice(5, 10) || '—'}
                        </td>
                      </tr>
                    )
                  })}
